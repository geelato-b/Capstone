...
include_once "../includes/db_conn.php";
    ...
    if(!mysqli_stmt_prepare($stmt_chk, $sql_check)){
       ...
       header("location: update.php?error=3"); //statement failed
        exit();
    }
    ...
    if(!empty($arr)){
        ...
        header("location: update.php?error=1&itemname={$accbty_name}"); //item exist
        exit();
    }
    else{
       $sql_ins="INSERT INTO `accountabilities`
        ...
        if(!mysqli_stmt_prepare($stmt_ins, $sql_ins)){
            ...
            header("location: update.php?error=2"); //insert failed
            exit();
        }
        ...
        header("location: update.php?error=0"); //success
    }
}
